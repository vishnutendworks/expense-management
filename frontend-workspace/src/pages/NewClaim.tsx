...
interface ItemEntry {
  ...
  amount: string;
  tax: string;
  otherCharges?: string;  // Added Other Charges
  desc: string;
  billable: boolean;
...
export const NewClaim: React.FC = () => {
  ...
  const [items, setItems] = useState<ItemEntry[]>([
    {
      ...
      amount: '', tax: '', otherCharges: '', desc: '', // Added otherCharges init
      billable: false,
      currency: 'INR',
  ...
  const getCategoryBreakdown = () => {
    ...
    items.forEach(item => {
      ...
      // Added otherCharges to category breakdown math
      const amt = parseFloat(item.amount || '0') + parseFloat(item.tax || '0') + parseFloat(item.otherCharges || '0');
  ...
  const calculateTotal = () => {
    // Added otherCharges to grand total calculation
    const total = items.reduce((sum, item) =>
      sum + (parseFloat(item.amount) || 0) + (parseFloat(item.tax) || 0) + (parseFloat(item.otherCharges) || 0), 0
    );
        ...
        setItems([
          {
            ...
            tax: taxAmount,
            otherCharges: otherCharges, // Bound to state
  ...
  const triggerLineItemOcr = async (file: File, idx: number) => {
    ...
    try {
      ...
      if (response.ok && data.status === 'success' && data.extracted_data) {
        ...
        // Extracted other_charges from backend response for multiline items
        const otherCharges = (extracted.other_charges ?? '').toString();
  ...
  const evaluateRoutingPath = (): 'pathA' | 'pathB' | 'pathC' => {
    // Math updated to include otherCharges
    const totalVal = items.reduce((sum, item) => sum + (parseFloat(item.amount) || 0) + (parseFloat(item.tax) || 0) + (parseFloat(item.otherCharges) || 0), 0);
    const maxLineVal = items.reduce((max, item) => Math.max(max, (parseFloat(item.amount) || 0) + (parseFloat(item.tax) || 0) + (parseFloat(item.otherCharges) || 0)), 0);
                    ...
                    {items.map((item, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Row 1: 5 Columns to fit Other Fees/Tolls */}
                          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                            ...
                            {/* 5th Column for Other Fees */}
                            <div className="space-y-1.5">
                              <label className="text-[11px] font-black text-slate-500 uppercase tracking-wider">Other Fees</label>
                              <div className="relative">
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-bold">{currSymbol}</span>
                                <input
                                  type="number"
                                  ...
                                  onChange={(e) => {
                                    const newItems = [...items];
                                    newItems[idx].otherCharges = e.target.value;
                                    setItems(newItems);
                                  }}
                                  placeholder="0.00"
                                  className="w-full pl-7 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none font-bold"
                      ...
                      <div className="text-right">
                        ...
                        <h4 className="text-3xl font-black text-slate-900">{(() => {
                          ...
                          return items.map(it => {
                            ...
                            const amt = (parseFloat(it.amount || '0') + parseFloat(it.tax || '0') + parseFloat(it.otherCharges || '0'));
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-slate-50 rounded-2xl p-6 space-y-4">
                          ...
                          <ReviewRow
                            label="Category"
                            value={
                              claimType === 'multiline' && new Set(items.map(item => item.category)).size > 1
                                ? 'Multiple Categories'
                                : reportCategory
                            }
                          />
                          ...
                          {items.map((item, idx) => (
                            <div key={item.id} className="bg-slate-50 rounded-2xl px-6 py-4 space-y-2">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs font-black text-slate-900">{(() => { const CURRENCY_SYMBOLS: Record<string, string> = { INR: '₹', USD: '$', EUR: '€', GBP: '£', AED: 'د.إ' }; return (CURRENCY_SYMBOLS[item.currency || 'INR'] ?? item.currency) + (parseFloat(item.amount || '0') + parseFloat(item.tax || '0') + parseFloat(item.otherCharges || '0')).toLocaleString('en-IN'); })()}</span>
                              </div>
                              ...
                              <div className="grid grid-cols-4 gap-2">
                                ...
                                <ReviewRow label="Other Fees" value={`${(() => { const S: Record<string, string> = { INR: '₹', USD: '$', EUR: '€', GBP: '£', AED: 'د.إ' }; return S[item.currency || 'INR'] ?? item.currency; })()}${parseFloat(item.otherCharges || '0').toLocaleString('en-IN')}`} />
