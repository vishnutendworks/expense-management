...
export const NewClaim: React.FC = () => {
  ...
  const clearReceiptFile = () => {
    ...
    setTripEndDate('');
    setItems([
      { id: Date.now(), date: new Date().toISOString().split('T')[0], category: 'Travel Expenses', amount: '', tax: '', desc: '', billable: false, currency: 'INR', paymentMode: 'Personal Card', projectCode: '', merchantName: '' }
    ]);
  ...
  const handleReceiptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Auto-fill the form
      const merchant = d.merchant_name || 'Unknown Merchant';
      if (merchant) setClaimTitle(`Receipt – ${merchant}`);
      if (d.expense_date) {
        setTripStartDate(d.expense_date);
        setTripEndDate(d.expense_date);
      }
      if (d.category) {
        setReportCategory(d.category);
      }
      // NEW: Catch the Invoice ID from the OCR response
      if (d.invoice_id) {
      ...
      console.error('[OCR] Network/parse error:', err);
      setOcrError(message);
  ...
  const saveDraft = () => {
    const draftClaim: Claim = {
      ...
      riskCategory: 'medium',
      bankStatementReconciled: 'Unverified'
    };
    addClaim(draftClaim);
    navigate('/my-claims');
  };
  ...
  const evaluateItemPolicy = (item: ItemEntry): { status: 'pass' | 'warning' | 'error'; message: string } => {
    ...
    if (!catRule) {
      ...
      if (amountVal2 > policy.limit) {
        return {
          status: 'error',
          message: `Exceeds the category limit of ₹${policy.limit.toLocaleString('en-IN')} by ₹${(amountVal2 - policy.limit).toLocaleString('en-IN')}. Requires justification.`
        };
      }
                        ...
                        <div className="mt-4">
                          ...
                          {!receiptFile ? (
                            ...
                            <div className="mt-2 flex items-center gap-2">
                              <div className="flex-1 text-[10px] text-slate-700 font-black bg-[#FAF8F3] border border-slate-200 p-2.5 rounded-xl flex items-center gap-1.5 min-w-0">
                    ...
                    <AnimatePresence>
                      {aiOcrStatus !== 'idle' && (
                        <motion.div
                          ...
                          className={`p-5 rounded-2xl space-y-3 shadow-xl ${aiOcrStatus === 'error' ? 'bg-rose-950 text-white' : 'bg-slate-900 text-white'
                            }`}
                        >
                          <div className="flex items-center justify-between">
                            <p className={`text-xs font-black uppercase tracking-widest flex items-center gap-2 ${aiOcrStatus === 'error' ? 'text-rose-400' : 'text-yellow-400'
                              }`}>
                              ...
                              {ocrConfidence !== null && (
                                <div className="flex items-center gap-2">
                                  <div className="flex-1 bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                    <div
                                      className={`h-full rounded-full ${ocrConfidence >= 0.8 ? 'bg-emerald-400' :
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="p-4 border border-slate-200 rounded-xl bg-slate-50 hover:border-black transition-all">
                              ...
                              <button
                                ...
                                className={`w-full py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer ${item.receiptFile
                            ...
                            <div className="p-4 border border-slate-200 rounded-xl bg-slate-50 hover:border-blue-650 transition-all">
                              ...
                              <button
                                ...
                                className={`w-full py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer ${item.bankFile
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div className="space-y-3">
                ...
                <LiveCheck
                  ...
                  desc={`Trust score: ${userGradeTrust === 'low' ? 30 : (userGradeTrust === 'high' ? 95 : userTrustScore)}% (${(userGradeTrust === 'low' ? 30 : (userGradeTrust === 'high' ? 95 : userTrustScore)) >= 80 ? 'High' : (userGradeTrust === 'low' ? 30 : (userGradeTrust === 'high' ? 95 : userTrustScore)) >= 55 ? 'Moderate' : 'Requires Review'})`}
                />
              </div>
            </div>
          </div>
        ...
        {isSubmitting && (
          ...
          >
            <div className="bg-[#FAF8F3] rounded-3xl p-8 max-w-lg w-full mx-4 shadow-2xl border border-slate-100 space-y-6">
              ...
              <div className="space-y-4">
                <RoutingPipelineStep title="1. Spending Policy Validation" active={routingStep === 1} completed={routingStep > 1} icon={ShieldCheck} />
                <RoutingPipelineStep title="2. Anomaly & Duplicate Detection" active={routingStep === 2} completed={routingStep > 2} icon={AlertTriangle} />
                <RoutingPipelineStep title="3. Mathematical Statement Reconciliation" active={routingStep === 3} completed={routingStep > 3} icon={FileCheck} />
                <RoutingPipelineStep title="4. Submitter Trust Index Calculation" active={routingStep === 4} completed={routingStep > 4} icon={UserCheck} />
              </div>
